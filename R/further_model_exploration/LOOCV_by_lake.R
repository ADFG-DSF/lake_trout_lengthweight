...
########## loading data and setting up for mode ###########
...
#### defining the three models we're realistically considering
...
cat('model {
  ...
  for(j in whichlakes_WL) {
    b0[j] ~ dnorm(mu_b0[j], tau_b0)
    b0_interp[j] <- b0[j] - b1[j]*meanlogLc
    mu_b0[j] <- b0_int + b0_area*logareac[j]
...
# ncores <- 3
# ncores <- 8  # commented this out so it will match loaded file
...
### running the three models, TAKING OUT THE WEIGHT ~ LENGTH data for each lake in turn
### then comparing RMSE of predicted log weights vs actual log weights for each lake

# the subset of lakes that actually have weights
...
Linf_1 <- Linf_2 <- Linf_3 <- Winf_1 <- Winf_2 <- Winf_3 <-
  matrix(nrow=nrow(Linf_0), ncol=ncol(Linf_0))

# initializing and looping
...
for(i in seq_along(theselakes)) {
  ...
  outs <- jagsUI::jags(model.file=model1, data=data_i,
                       parameters.to.save=c("logW","Linf","Winf"),
  ...
  Linf_1[,i] <- outs$sims.list$Linf[,theselakes[i]]
  Winf_1[,i] <- outs$sims.list$Winf[,theselakes[i]]
  ...
  Linf_2[,i] <- outs$sims.list$Linf[,theselakes[i]]
  Winf_2[,i] <- outs$sims.list$Winf[,theselakes[i]]
  ...
  Linf_3[,i] <- outs$sims.list$Linf[,theselakes[i]]
  Winf_3[,i] <- outs$sims.list$Winf[,theselakes[i]]
...
nweights <- laketrout_Winf$n_Weight[laketrout_Winf$n_Weight > 1]
mm <- function(x) x-min(x, na.rm=TRUE)
...
abline(0,1)
plot(apply(Winf_0, 2, median), apply(Winf_2, 2, median))
abline(0,1)
...
cbind(
  ...
  abs(apply(Linf_0, 2, median)-apply(Linf_2, 2, median)),
