...
```{r, fig.width=6.5, fig.height=2.18, warning=FALSE}

par(mar=c(2.7, 2.5, 3.5, 0.3)+0.1)
par(mgp=c(1.7, 0.5, 0))
par(tcl=-.2)
par(family="serif")
# par(cex.axis=1.2)
...
pointcol <- adjustcolor(1, alpha.f=.5)
curvecol <- adjustcolor(1, alpha.f = .2)
envcol <- adjustcolor(1, alpha.f=0.7)

lakenames[lakenames=="Summit Lake (Richardson Highway near Paxson"] <-
  "Summit Lake (Richardson Hwy)"

for(i in theselakes) {
  par(xpd=FALSE)

  # L ~ Age
  plot(NA, xlab="Age", ylab="Length (mm)", 
       # main=lakenames[i],
       xlim=c(0, max(int_Winf_data$Age, na.rm=TRUE)),
       ylim=c(0, max(int_Winf_data$L[!is.na(int_Winf_data$Age)], na.rm=TRUE)),
       yaxt="n")
  title(lakenames[i], adj=0, xpd=NA)
  for(j in 1:ncol(int_Winf_jags_out$q50$Lfit)) {
    lines(int_Winf_jags_out$q50$Lfit[,j], col=curvecol, lty=3)
  }
  if(i <= dim(int_Winf_jags_out$sims.list$Lfit)[3]) {
    envelope(int_Winf_jags_out$sims.list$Lfit[,,i], add=TRUE, col=envcol)
  }
  points(x=int_Winf_data$Age[int_Winf_data$lake==i],
         y=int_Winf_data$L[int_Winf_data$lake==i],
         col=pointcol)
  jagshelper:::smashaxis(2, at=seq(from=0, to=800, by=200), smashall=TRUE)


  # W ~ Length
  plot(NA, xlab="Length (mm)", ylab="Weight (kg)", main="", 
       # log="xy",
  ...
  envelope(exp(int_Winf_jags_out$sims.list$b0_interp[,i]) *
              ...
              x=lvec, add=TRUE, col=envcol)
  jagshelper:::smashaxis(2, at=seq(from=0, to=12, by=2), smashall=TRUE)
  ...
  barheights <- seq(to=-4, by=5.5, length.out=4)-1
  ...
  text(x=-8.5, y=barheights, pos=4, cex=1.2, col=cols,
