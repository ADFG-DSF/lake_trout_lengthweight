...
Note: Lengths were recentered by subtracting the mean measurement, in order to eliminate collinearity between the intercept and slope parameters and thus aid model convergence.  Raw estimates of the intercept parameters appear quite different from that reported by Lester, et al.; these were transformed for the sake of interpretability and consistency.  Latitude and log area were also recentered by subtracting the respective means.
...
Dotted horizontal lines represent values reported by Lester, et al.  Note that parameter estimates are similar to those reported by Lester, et al., but in many cases are substantially different from one another, suggesting variation in weight ~ length relationships between lakes.
...
In this case, the parameter of most inferential interest was asymptotic length $L_{\infty}$.  Hyperparameter $\mu_{L_{\infty}}$ was given a weakly informative prior of $N(700, 100)$, and $\mu_{t_0}$ was mildly constrained with a prior of $N(0,1)$.  All standard deviation $\sigma_.$ parameters were modeled with uniform priors on the SD scale, constrained as appropriate.
...
* However, since many lakes have much samples for length than for weight or age, it is possible that methods ranked as less preferable might result in better estimates.
...
A few outlying measurements of either length or weight were noted.  These observations were censored rather than making assumptions at possible transformations.
