...
In order to effectively manage resident lake trout *Salvelinus namaycush* in Alaska, sustainable yield must be estimated from some form of productivity model.  One such model shows promise, but is particularly mathematically sensitive to estimation of asymptotic weight $W_{\infty}$.  Information from lake area is currently used as a surrogate for $W_{\infty}$, but model inferences may be dramatically improved by means of direct estimation of $W_{\infty}$. 
...
2. Further estimate the length ~ age relationship in terms of Von Bertalanffy parameters *k* and $t_0$; and

3. Estimate the weight ~ length relationship in terms of log-log regression parameters $\beta_0$ and $\beta_1$;
...
Sampling data were used from `r sum(nmat_num$n_Projects)` unique sampling efforts, across `r sum(nmat_num$Use_Samples=="Yes")` lakes deemed to have sufficiently representative and high-quality data.  Out of `r nlakes` lakes of interest with lake trout present as a resident species, estimation of asymptotic size is desired by management for `r sum(nmat_num$Estimate_Winf=="Yes")` lakes, `r sum(nmat_num$Estimate_Winf=="Yes" & nmat_num$Use_Samples=="Yes")` of which may make direct use of sampling data and `r sum(nmat_num$Estimate_Winf=="Yes" & nmat_num$Use_Samples=="No")` of which have not yet been sampled.  After filtering to the most reliable data (Appendix B), a wealth of data remain, and are summarized for each lake in Table \@ref(tab:nmat).
...
Growth parameters $L_{\infty}$, $k$, and $t_0$ were modeled separately for each lake *j*, with hierarchical distributions according to the form below.  Note that the hierarchical mean for $t_0$ was mildly constrained to be near zero; this was to aid convergence and to ensure that it fell within a biologically reasonable range.
...
Growth parameter $L_{\infty}$ may be thought of as hierarchically distributed, but with a central trend defined not by a global mean but according to a relationship with lake area of the same form as that reported by Lester, et al., and with multiplicative (lognormal) process error.  Parameters $\gamma$ and $\lambda$ were given weakly informative priors centered on the values reported by Lester, et al., and with standard deviations determined by the coefficients of variation of 100% and 250%, respectively.  For lake *j*:
...
Lake-level regression parameters $\beta_0$ and $\beta_1$ were modeled explicitly, as opposed to the addition of a lake-level offset to the regression equation.  This allowed more straightforward modeling of the central trends of intercept and slope parameters as regression-type functions of lake-level variables as appropriate (see Appendix C).

Diffuse Uniform priors were used for standard deviation parameters $\sigma_{WL}$, $\sigma_{\beta_0}$, and $\sigma_{\beta_1}$.

Log-transformed lengths were recentered by subtracting the overall mean in order to eliminate correlation between $\beta_0$ and $\beta_1$ and therefore aid model convergence.  Inferences for $\beta_0$ were therefore back-transformed for the purpose of interpretability and equivalence with the relationship presented by Lester, et al., and the back-transformed parameters will be presented hereafter.
...
The relationship between length and age was estimated for `r sum(!is.na(int_Winf_jags_out$q50$k))` lakes, thus directly modeling the asymptotic length parameter $L_{\infty}$.  Additional LVB parameters *k* and $t_0$ were modeled hierarchically, with inferences shown in Figures \@ref(fig:kplot) and \@ref(fig:t0plot). 
...
Estimates for asymptotic size and lake-level parameters are presented in Table \@ref(tab:pmat).
...
```{r nmat, tab.id="nmat", tab.cap="Available sampling data for all lakes."}
# nmat1 <- nmat
...
```{r pmat, tab.id="pmat", tab.cap="Estimates of asymptotic size and lake-specific growth and body morphometry parameters."}
pmat1 <- pmat
